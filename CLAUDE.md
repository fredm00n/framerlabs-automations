...
- **Source:** Framer's Next.js RSC endpoint — fetched directly with `Rsc: 1` header, returns structured component data (no headless browser needed). The **actual newest-templates grid** (the list a visitor sees under "Newest") is embedded in the RSC stream in named section arrays: `"freshFinds"` (newest), `"trending"`, `"free"`, `"rotatingCategory"`, `"rotatingFeature"`. Each section delivers its templates in an `"items":[]` array whose elements are full template objects (`{"type":"template",…}`). The parser reads all `"items":[]` occurrences (`_parse_rsc_data_array`). A *separate* set of 12 curated `"resource":{…}` **featured** blocks is also parsed and merged (deduplicated by slug). **Parsing only the featured `"resource"` blocks silently misses every newly published template**, since the featured carousel rarely changes; the `"items":[]` section arrays are the authoritative newest feed. The older June 2026 `"data":[]` array format is also searched as a fallback in case Framer reverts.
...
- **RSC format (June 2026+, sectioned July 2026+):** Framer changed the RSC payload structure in June 2026, then again in July 2026. Each template carries `attributes.price` (a **number**, or `null` for free — unlike the featured blocks' string price), `attributes.previewUrl`, `author.name/slug`, `media[0].url` for thumbnail, `introduction` for meta title, and plain ISO 8601 `publishedAt` (no `$D` prefix). Field mapping is shared by both parse paths via `_new_format_template`. Price is normalised by `_format_price` to a display string — `"$79"` for paid, `"Free"` for free — so the numeric/`null` section-array price renders with a currency symbol (not a bare `79`) and free templates show `"Free"` rather than a blank. The pre-2026 `"item":` key and field layout are kept as an automatic fallback so the parser recovers if Framer rolls back the format change. The June 2026 `"data":[]` single-array format is also tried for backward compatibility.

**Deferred improvements** (still open):
- RSC format fragility — Framer could change the response structure without notice; the `"items":[]` section arrays, the `"data":[]` fallback, the featured `"resource"` blocks, and the legacy `"item"` keys are all tried, plus diagnostics, but a completely new encoding would need manual intervention
