...
- RSC format is an internal Next.js mechanism — Framer could change the response structure without notice. When parsing yields < 5 templates a Discord alert is sent to `DISCORD_ALERTS_WEBHOOK_URL` and a `body_preview` (first 500 chars of the last page) is logged to `logs/errors.jsonl` to aid diagnosis; inspect the raw RSC payload and update `_extract_json_object` / the search key if needed
