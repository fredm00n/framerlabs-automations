...
- Cross-run Discord alert suppression — implemented: a sustained Framer-side outage (RSC parse failures, < 5 templates) or Notion misconfiguration (`get_seen_slugs` HTTP error) previously fired one identical Discord alert per 15-min cron tick, flooding the alerts channel with redundant notifications carrying no new diagnostic info. `_warn_discord` now accepts an optional `dedup_key`; on a successful POST the send timestamp is recorded to `state/alert_state-framer_templates.json`, and subsequent calls within `_ALERT_SUPPRESS_MINUTES = 60` are silently skipped. Recording only happens on a successful Discord POST so a transient 5xx does not suppress the next attempt when the webhook recovers. Each alert site uses a stable key (`framer_templates:fetch_failed`, `framer_templates:few_templates_parsed`, `framer_templates:seen_slugs_http_error`, `framer_templates:seen_slugs_other_error`). State path is script-specific (`state/alert_state-framer_templates.json`, separate from `state/alert_state-reddit_leads.json`) so the two scripts' commits do not race on a single file when the parallel `reddit-leads-monitor` workflow pushes. Mirrors the same suppression contract already in `reddit_leads.py`. Twelve new tests cover the suppression helpers (load/record/window logic, corruption handling, key isolation, per-script state path) and `_warn_discord`'s dedup contract (first send records, second within window skipped, no-key never suppresses, failed send does not record).
...
- Consecutive save-failure short-circuit — implemented: `main()` now tracks a `consecutive_save_failures` counter that resets to 0 on any successful `save_to_notion` call. When it reaches `_CONSECUTIVE_SAVE_FAILURE_SHORT_CIRCUIT = 3`, the save loop breaks, a WARNING-level Discord alert fires (dedup key `framer_templates:save_short_circuited`), and a structured warning is logged to `logs/errors.jsonl` with `consecutive_save_failures`, `saved`, `total_new`, and `skipped` counts. Without this, a Notion outage during a 20-template batch burned ~14 s per template in retried timeouts (4 attempts x exponential backoff 2s+4s+8s), totalling ~280 s of wasted work with no useful outcome and no operator-visible alert specific to the save phase. Templates saved before the short-circuit are still notified via Discord and X. Mirrors the same pattern used by `reddit_leads.py` (`_CONSECUTIVE_DEDUP_FAILURE_SHORT_CIRCUIT`). Eight new tests cover: three consecutive failures short-circuit; Discord alert fires; log context includes counts; alternating success/failure does not short-circuit; counter resets after success; HTTPError triggers short-circuit; successfully saved templates still notified; first-run suppression preserved.
...
- `'rate my'` in `_ALWAYS_EXCLUDE` matched as plain substring of `'migrate my'` — implemented: moved `'rate my'` out of `_ALWAYS_EXCLUDE` (plain substring match) into a new `_ALWAYS_EXCLUDE_WORD_START_PHRASES` frozenset checked via a new `_has_word_start_phrase()` helper, which uses `\b` before the first word of each phrase so `'rate'` as a whole word matches `'rate my design/website'` but not `'migrate my website'` / `'migrate my site'` (where `'rate'` is a suffix of `'migrate'`). Without this fix the textbook high-intent hiring post "[Hiring] need to migrate my website from Squarespace to Framer, paid" was silently dropped — exactly the lead the reviewer most wants to see. The same defect was previously identified and fixed in `dub-affiliate-scanner/scripts/reddit_scanner.py`; this change mirrors that pattern. Ten new tests cover: the original `'rate my design/website'` exclusion still fires; `'migrate my website/site'` hiring posts now pass (one in r/forhire under the WEB_SIGNALS gate, one in r/startups under the BUSINESS_WEB + HIRE gate); `_has_word_start_phrase` matches `'rate my'` at start-of-string and after a word boundary; `_has_word_start_phrase` rejects `'migrate my'`, `'celebrate my'`, and `'berate my'`; a regression guard asserts `'rate my'` lives in `_ALWAYS_EXCLUDE_WORD_START_PHRASES` (not back in `_ALWAYS_EXCLUDE`) so a future refactor cannot silently re-introduce the bug.
...
- Fetch-failure Discord alert samples — implemented: `fetch_reddit_posts` now accepts an optional `error_samples: list[str] | None = None` parameter and, when provided, appends a short descriptor on every failure path (`r/<sub> HTTP <code>` for HTTP errors, `r/<sub> <ExceptionName>` for `URLError` / `TimeoutError` / generic exceptions, `r/<sub> ParseError` for malformed XML). `main()` passes a fresh list, then surfaces the first 5 entries in the existing `>50%` / `100%` fetch-failure Discord alert as `Samples: <a>; <b>; ...`. Without this, an alert like `"Majority of subreddit feeds failed (25/43)"` gave no signal about the dominant root cause — the operator had to open `logs/errors.jsonl` to discover whether it was a Reddit-wide outage (all `HTTP 500`, observed on 2026-05-18 with the "reddit broke!" HTML page), rate-limiting (all `HTTP 429`, throttle harder), a network blip at the runner (all `URLError`, transient), a partial outage of a specific Reddit shard (mixed status codes), or a parsing regression (all `ParseError`, code change needed). With the samples inline, the most likely action is visible at a glance from the Discord channel alone. The samples are capped at 5 to keep the alert readable; the full per-feed context still goes to `logs/errors.jsonl` for deeper inspection. The `samples` list is also written to the structured `logs/errors.jsonl` entry so post-hoc analysis is straightforward. Mirrors the diagnostic pattern already used by the dedup-failure alert (`dedup_error_samples`). The `error_samples` parameter is optional (defaults to `None`) so existing tests calling `fetch_reddit_posts(subreddit, feed_url)` directly continue to work unchanged.
- `body_preview` capture on XML `ParseError` — implemented: when `fetch_reddit_posts` catches `xml.etree.ElementTree.ParseError`, the first 500 characters of the raw response body are now logged as `body_preview` in `logs/errors.jsonl` alongside the existing `error` field. Without this the log entry only contained the parse-position message (e.g. `"syntax error: line 1, column 0"`) which gives no signal about *what* Reddit actually returned: the HTML "reddit broke!" outage page (Reddit serves this with HTTP 200 in some shard-outage modes, not always 500 — so the body would not have been captured by the existing HTTPError branch), a JSON-formatted rate-limit response, an authentication/captcha-challenge HTML page, or some other non-Atom payload. The capture mirrors the same `body_preview` field already populated in the `HTTPError` branch above it and matches the diagnostic pattern used by `save_lead_to_notion`, `url_exists_in_notion`, `notify_discord_lead`, etc. An empty body (rare but possible — a transient proxy/CDN bug) is handled defensively (`body_preview` becomes `''` rather than raising). The 500-char cap keeps `errors.jsonl` lines manageable even when Reddit returns a large HTML page.
- HTTPError `body_preview` truncation increased from 200 to 500 chars — implemented: `fetch_reddit_posts`'s HTTP error branch truncated the response body to 200 chars while the XML `ParseError` branch directly below it and every other body capture in both scripts (`save_lead_to_notion`, `url_exists_in_notion`, `notify_discord_lead`, `_warn_discord`, `save_to_notion`, `post_to_x`, `notify_discord_batch` in `framer_templates.py`) used 500 chars. The inconsistency was a residual from an earlier version. The May 18 Reddit outage logs demonstrate the impact: all 25 HTTP 500 entries have `body_preview` cut off at exactly 200 chars, losing the diagnostic `<body>` content of the "reddit broke!" HTML page that would have been visible at the 500-char cap used everywhere else. Aligned to `[:500]` so the HTTPError and ParseError branches are consistent and operators can identify the failure mode at a glance from `logs/errors.jsonl`. Two new tests cover: (1) the HTTPError path captures the response body as `body_preview`; (2) the capture is capped at 500 chars and exceeds the old 200-char limit.
- Early-exit on Notion `object_not_found` dedup failure — implemented: `main()` now sets a `db_misconfigured` flag the first time a dedup HTTPError's response body contains `"object_not_found"`, and the outer subreddit-feed loop breaks on the next iteration check.  Once Notion returns `object_not_found` for the configured `NOTION_REDDIT_LEADS_DB_ID`, the DB is misconfigured (deleted, renamed, or no longer shared with the integration) and **every** subsequent dedup query and every save attempt in the run will fail the same way — there is no useful work left to do.  Without the short-circuit, the script keeps fetching all 43 Reddit RSS feeds (each paced at `_INTER_FEED_DELAY = 1.5s` plus up to 30s of HTTP timeout × retry attempts) and re-runs the doomed dedup check once per filtered post, consuming most of the 15-minute cron window on a guaranteed-failure path and flooding `logs/errors.jsonl` with N redundant warnings (each adding nothing diagnostic beyond the first).  With the early-exit, the single ERROR-level Discord alert at the end of `main()` still fires (the `dedup_object_not_found_errors` counter and `dedup_error_samples` list captured at the first occurrence are exactly what the alert path consumes), but only one warning entry is written to the log file and the cron run finishes in seconds instead of minutes.  Scoped narrowly: the early-exit only triggers on `object_not_found` specifically — other 404s (Cloudflare-style HTML pages, transient Notion outages that strip the error code from the body) and timeouts/5xx do **not** trip it, because those failure modes are individually transient and the next post or next subreddit may well succeed.  Two new tests cover: (1) one feed-fetch + one dedup call after the first `object_not_found` (proving the outer loop breaks); (2) a 404 whose body lacks `object_not_found` still inspects all 43 feeds (proving the guard is correctly scoped).
- Generalised dedup-failure short-circuit — implemented: the 2026-05-21 incident showed PR #92's narrow `object_not_found` short-circuit did not cover the actual failure modes Notion presented (`TimeoutError`, generic `Exception`, non-404 5xx).  All 43 dedup calls failed in each affected run, each burning up to ~120s of retried timeouts, exceeding the 15-min cron window.  `main()` now also tracks a `consecutive_dedup_failures` counter that resets on any successful dedup or save; once it reaches `_CONSECUTIVE_DEDUP_FAILURE_SHORT_CIRCUIT = 3`, the run sets `notion_likely_down = True` and breaks the outer feed loop.  A dedicated WARNING-level alert ("`Notion appears unreachable. Short-circuited the run...`") fires at the end, distinct from the `object_not_found` ERROR alert so the operator can tell DB-misconfiguration from in-flight outage at a glance.  Three new tests cover: (1) three consecutive `TimeoutError`s short-circuit; (2) three consecutive 5xx HTTPErrors short-circuit; (3) alternating fail/fail/success across all 43 feeds does **not** short-circuit (the counter resets on each success).
- Notion preflight at start of `main()` — implemented: a single cheap query against the configured DB (`_notion_preflight`) runs before the per-feed loop.  When Notion is broadly unreachable, this catches the failure in ~2.5 min worst case instead of letting it cascade across 43 doomed dedup calls.  On failure, a single alert fires (DB-misconfigured wording for `object_not_found`, generic "Notion preflight failed" wording for everything else) and the run exits before any Reddit traffic.  Uses a direct `http_post` call rather than going through `url_exists_in_notion` so test patches targeting in-flight dedup behaviour are not also exercised by the preflight — keeps the test surface small.  Three new integration tests cover preflight-pass-then-loop, preflight-`object_not_found`-exits, and preflight-`TimeoutError`-exits, plus five direct tests for the `_notion_preflight` helper itself.
- Cross-run Discord alert suppression — implemented: a sustained outage previously produced one identical Discord alert per 15-min cron tick (today's incident: 4 redundant alerts in ~30 min, none carrying new diagnostic info).  `_warn_discord` now accepts an optional `dedup_key`; on successful send, the timestamp is recorded to `state/alert_state-reddit_leads.json`, and subsequent calls within `_ALERT_SUPPRESS_MINUTES = 60` are silently skipped.  Recording only happens on a successful Discord POST so a transient 5xx does not suppress the next attempt when the webhook recovers.  Each alert site uses a stable key (`reddit_leads:fetch_all_failed`, `reddit_leads:fetch_majority_failed`, `reddit_leads:dedup_object_not_found`, `reddit_leads:dedup_notion_likely_down`, `reddit_leads:dedup_other_failures`, `reddit_leads:notion_preflight_object_not_found`, `reddit_leads:notion_preflight_other`).  State path is script-specific (`state/alert_state-reddit_leads.json`) so it does not race with the parallel `framer-monitor` workflow's commits.  Per-script paths also avoid the `git push` race that lost today's `logs/errors.jsonl` entries — see the workflow push-retry change below.  Eight new tests cover the suppression helpers (load/record/window logic, corruption handling, key isolation) and four new tests cover `_warn_discord`'s dedup contract (first send records, second within window skipped, no-key never suppresses, failed send does not record).
